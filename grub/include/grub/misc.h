...
#define grub_calloc(a,b) AllocateZeroPool((a) * (b))
#undef  calloc
#define calloc grub_calloc
#define grub_malloc(a) AllocateZeroPool(a)
#undef  malloc
...
#define grub_dprintf(condition, ...) grub_real_dprintf(GRUB_STRINGIFY(GRUB_FILE), __func__, __LINE__, condition, __VA_ARGS__)
...
#undef  grub_err_printf
#define grub_err_printf AsciiPrint
...
void EXPORT_FUNC(grub_real_dprintf) (const char *file, const char *function, const int line, const char *condition, const char *fmt, ...) __attribute__ ((format (GNU_PRINTF, 5, 6)));
